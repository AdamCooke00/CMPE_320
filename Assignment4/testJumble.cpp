...
void playGame() {
	...
	cout << "\n\nProvide a string to hide in the puzzle: ";
	cin >> toHide;
...
void testJumble() {
	...
	//PERSONAL UNIT TESTS
	//
	cout <<"\nSHOULD SAY WORD IS NOT BETWEEN 3-10 CHARACTERS" << endl;
	try
	{
		JumblePuzzle jp6("qwertyuiopa", "easy");
	}
	catch(BadJumbleException& e)
	{
		cout << e.what() << endl;
	...
	cout <<"\nSHOULD SAY DIFFICULTY IS NOT RECOGNIZED" << endl;
	try
	{
		JumblePuzzle jp7("word", "notadifficulty");
	}
	catch(BadJumbleException& e)
	{
		cout << e.what() << endl;
	...
	cout <<"\nSHOULD SAY HIDDEN WORD CONTAINS NUMBER(S), ENTER CHARACTERS ONLY" << endl;
	try
	{
		JumblePuzzle jp8("abc123abc", "easy");
	}
	catch(BadJumbleException& e)
	{
		cout << e.what() << endl;
